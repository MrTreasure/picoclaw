...
func TestTurnProfile_HistoryOffSuppressesHistoryAndPersistence(t *testing.T) {
	...
	if !strings.HasSuffix(provider.messages[1].Content, "new user") {
		t.Fatalf("current message = %q, want it to end with new user", provider.messages[1].Content)
	}
...
func TestTurnProfile_ProcessMessageUsesEnabledTurnProfile(t *testing.T) {
	...
	if !strings.HasSuffix(provider.messages[1].Content, "hello from pico") {
		t.Fatalf("current message = %q, want it to end with hello from pico", provider.messages[1].Content)
	}
}
